<template >
  <view-box v-ref:view-box class="reportStudent">
    <div slot="header" style="position:absolute;left:0;top:0;width:100%;z-index:100">
      <x-header :left-options="{showBack: true}">
         <change-text-book :value.sync="textbookId" :user-textbook="User.textbook" :subject-id="brushedSubjectId"></change-text-book>
  ...
  <gnb-change-sub :visible.sync="visible" :selected="brushedSubjectId" :subject="User.subjectType" @on-click-back="_changeSubject"></gnb-change-sub>
</template>
...
export default {
    ...
    data:function(transition){
      ...
      */
      if(this.brushedIsReset){
        ...
        this.$nextTick(() => {
          document.getElementsByClassName("vux-fix-safari-overflow-scrolling")[0].scrollTop = this.brushedScroll;
        })
      }
    }
  },
  methods: {
     ...
     _intoTextbook(){
       this.$router.go(`/main/bag/textbook/add?subjectId=${this.brushedSubjectId}`);
     },
     _toDetail(chapterId){
  ...
  data(){
	...
	computed:{
    ...mapGetters(['brushedScroll','brushedChapter','User','brushedSubjectId','brushedIsReset'])
	},
  watch: {
    textbookId(){
      this.clearBrushed();
