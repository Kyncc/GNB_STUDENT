/** 数学练习册 */
export const workbookMath = (state) => state.workbook.math
/** 物理练习册 */
export const workbookPhysics = (state) => state.workbook.physics
/** 全部练习册 */
export const workbookAddList = (state) => state.workbook.add
/** 搜索练习册 */
export const workbookSearchList = (state) => state.workbook.search
/** 想要练习册 */
export const workbookWant = (state) => state.workbook.want
/** 上传作业 */
export const workbookUploader = (state) => state.uploader
/** 章节 */
export const workbookChapter = (state) => state.chapter
/** 练习 */
export const workbookExercise = (state) => state.exercise
