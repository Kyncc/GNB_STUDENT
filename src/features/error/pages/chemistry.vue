<template>
  <div>
    <card v-for='(error, index) in errorChemistry.list' :key='index'>
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
      if (vm.errorChemistry.isReset) {
        vm._getData()
      }
      vm.$parent.$refs.viewBoxBody.scrollTop = vm.errorChemistry.scroll
    })
  },
