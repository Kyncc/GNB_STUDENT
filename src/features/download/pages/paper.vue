<template>
  <view-box body-padding-top="46px">
    <x-header slot="header" style="width:100%;position:absolute;left:0;top:0;z-index:1;" :left-options="{backText: '试卷下载'}">
      <div slot="right" style="margin:0">
        <gnbChangeSub :change.sync='subject'></gnbChangeSub>
      </div>
    </x-header>
    <div>
      <group :gutter='0' title='只展示最近一个月的试卷' v-show='DownloadPaper.list.length'>
        <cell :title='item.name' v-for='(item, index) in DownloadPaper.list' :key='index'>
      ...
      <div style="text-align:center;padding:10px 0;">
        ...
        <p v-else-if="!DownloadPaper.list.length" style="font-size:16px;color:#4cc0be">近一月还未有下载记录</p>
...
export default {
  ...
  methods: {
    ...mapActions(['getDownloadPaperHistory', 'clearDownload']),
    _download (item) {
    ...
    _getData () {
      ...
      this.loading = true
      this.getDownloadPaperHistory({subject: this.subject}).then(() => {
        this.error = false
        this.loading = false
      }).catch((e) => {
        this.error = true
