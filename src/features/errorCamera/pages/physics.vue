<template>
  <div>
    <card v-for='(error, index) in errorCameraPhysics.list' :key='index'>
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
      if (vm.errorCameraPhysics.isReset) {
        vm._getData()
      }
      vm.$parent.$refs.viewBoxBody.scrollTop = vm.errorCameraPhysics.scroll
    })
  },
  beforeRouteLeave (to, from, next) {
    this.setErrorScroll({subject: 'physics', height: this.$parent.$refs.viewBoxBody.scrollTop})
