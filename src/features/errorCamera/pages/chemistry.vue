<template>
  <div>
    <card v-for='(error, index) in errorCameraChemistry.list' :key='index'>
...
export default {
  ...
  computed: {
    ...mapGetters(['errorCameraChemistry'])
  },
  data () {
    return {
      loading: false,
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
      if (vm.errorCameraChemistry.isReset) {
        vm._getData()
      }
      vm.$parent.$refs.viewBoxBody.scrollTop = vm.errorCameraChemistry.scroll
    })
  },
  beforeRouteLeave (to, from, next) {
    this.setErrorCameraScroll({subject: 'chemistry', hight: this.$parent.$refs.viewBoxBody.scrollTop})
