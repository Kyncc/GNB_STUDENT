<template>
  <div>
    ...
    <group gutter="0" class="gnb_collapse" v-if="!loading">
      <template v-for="(list, index) in CameraChapterPhyscics.list.chaper">
...
export default {
  ...
  computed: {
    ...mapGetters(['CameraChapterPhyscics', 'User']),
    textList () {
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
      vm.$parent.$refs.viewBoxBody.scrollTop = vm.CameraChapterPhyscics.scroll
    })
  },
