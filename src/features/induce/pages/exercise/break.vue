<template>
  <div>
    <aside v-if="list.length" style="padding:0 .5rem;font-size:.7rem;line-height:1.5rem;margin-bottom:-10px;">
      <p>本节共有<b style="color:#F8BF4D">{{totalCount}}</b>个题型，已斩<b style="color:#F8BF4D">{{recordSize}}</b>题</p>
    </aside>
    ...
    <div style="text-align:center;padding:20px 0;">
      ...
      <div>
        <p style="font-size:16px;color:#4BB7AA" v-if="loadingNoData">已经加载全部斩题~</p>
...
export default {
  ...
  computed: {
    ...mapGetters(['Route', 'induceBreak']),
    list () {
      return this.induceBreak.list
    },
    recordSize () {
      return this.induceBreak.recordSize
    },
    totalCount () {
      return this.induceBreak.totalCount
    }
  },
  data () {
    return {
  ...
  methods: {
    ...
    _getData () {
      this.loading = true
      this.getInduceList({type: 'break'}).then((res) => {
    ...
    _back (item, index) {
      this.induceBack({index: index, type: 'break', id: item.exercises_id, chapter_id: item.chapter_id}).then(() => {
  ...
  beforeRouteLeave (to, from, next) {
    this.setInduceListScroll({type: 'break', height: this.$parent.$refs.viewBoxBody.scrollTop})
    next()
  }
}
</script>
