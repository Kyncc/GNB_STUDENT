<template>
  <div>
    <card v-for="(item, index) in list" :key="index">
      ...
      <div slot="content" @click="$router.push({name:'example', params: {subjectId: '2', id: item.exercises_id, from: item.from}})">
...
export default {
  ...
  methods: {
    ...
    _getData () {
      this.loading = true
      this.getCollect({id: 2, subject: 'math'}).then((res) => {
  ...
  beforeRouteLeave (to, from, next) {
    this.setCollectScroll({subject: 'math', height: this.$parent.$refs.viewBoxBody.scrollTop})
    next()
  }
}
</script>
